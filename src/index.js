...
class ChartComponent extends React.Component {
  ...
  shouldComponentUpdate(nextProps) {
    ...
    if (height !== nextProps.height || width !== nextProps.width) return true;
    if (type !== nextProps.type) return true;
    if (!isEqual(legend, nextProps.legend)) return true;
    if (!isEqual(options, nextProps.options)) return true;

    const nextData = this.transformDataProp(nextProps);

    if (!isEqual(this.shadowDataProp, nextData)) return true;
